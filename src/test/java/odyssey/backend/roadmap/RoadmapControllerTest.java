...
class RoadmapControllerTest extends ControllerTest {
    ...
    void 로드맵을_전체조회한다() throws Exception {
        RoadmapResponse response1 = new RoadmapResponse(1L, "타이틀1", "설명1", List.of("테스트1", "테스트2"));
        RoadmapResponse response2 = new RoadmapResponse(2L, "타이틀2", "설명2", List.of("테스트1", "테스트2"));
    ...
    void 로드맵을_수정한다() throws Exception {
        Long roadmapId = 1L;
        RoadmapRequest request = new RoadmapRequest("수정된 타이틀", "수정된 설명", List.of("수정된 테스트1", "수정된 테스트2"));
        RoadmapResponse fakeResponse = new RoadmapResponse(roadmapId, request.getTitle(), request.getDescription(), request.getCategories());
